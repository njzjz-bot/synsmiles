...
from main.smiles_gfn.run_sql_base import (
    _build_seq_mask,
    _compute_sql_loss,
    _generate_sql_sequences,
    _get_special_token_ids,
)


class SQL_S3_Optimizer(BaseOptimizer):
    def __init__(self, args=None):
        super().__init__(args)
        self.model_name = "sql_s3"
    ...
    def _optimize(self, oracle, config):
        ...
        print("Model initialized, starting SQL-S3 training...")
        ...
        while True:
            ...
            model.train()
            loss, sql_stats = _compute_sql_loss(
                model,
                prior,
                valid_seqs,
                valid_seq_mask,
            ...
            valid_seq_logprobs = compute_sequence_logprobs(model, valid_seqs, tokenizer.pad_token_id)
            ...
            try:
                print(
                    f"sql_s3 {training_mode} kl {sql_stats['kl_mean']:.4f}, "
                    f"log_ratio {sql_stats['sample_log_ratio_mean']:.4f}, "
                    f"prior_lp {sql_stats['prior_action_logprob_mean']:.4f}"
                )
            except Exception:
                pass
            ...
            try:
                wandb.log(
                    {
                        "sql_s3/loss": float(loss.item()),
                        "sql_s3/aux_loss": float(aux_loss.item()),
                        "sql_s3/training_mode_is_replay": float(training_mode == "replay"),
                        "sql_s3/pos_replay_size": len(replay.heap),
                        "sql_s3/neg_replay_size": len(negative_replay.heap),
                        "sql_s3/synth_rate": float(synth_history[-1]) if synth_history else 0.0,
                        "sql_s3/avg_score": float(valid_scores.mean().item()) if valid_scores.numel() > 0 else 0.0,
                        "sql_s3/kl_mean": sql_stats["kl_mean"],
                        "sql_s3/sample_log_ratio_mean": sql_stats["sample_log_ratio_mean"],
                        "sql_s3/policy_action_logprob_mean": sql_stats["policy_action_logprob_mean"],
                        "sql_s3/prior_action_logprob_mean": sql_stats["prior_action_logprob_mean"],
                        "sql_s3/q_chosen_mean": sql_stats["q_chosen_mean"],
                        "sql_s3/soft_value_mean": sql_stats["soft_value_mean"],
                        "sql_s3/td_target_mean": sql_stats["td_target_mean"],
                    }
                )
            except Exception:
                pass
